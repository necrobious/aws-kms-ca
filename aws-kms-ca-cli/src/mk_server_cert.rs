...
fn dns_name_parser (input: &str) ->  Result<GeneralName, String> {
    Ok(GeneralName::DnsName(input.to_string()))
}

fn ip_addr_parser (input: &str) -> Result<GeneralName, String> {
    input.parse::<IpAddr>()
...
async fn main () -> Result<(), Box<dyn Error>> {
    ...
    // build Subject Alternative Name from ip-addrs and dns-names
    let gn = [&args.ip_addr[..], &args.dns_name[..]].concat();
    let san =
        if gn.len() > 0 {
