...
fn is_ip_addr (input:String) -> Result<(), String> {
    input.parse::<IpAddr>().map(|_| ()).map_err(|e| e.to_string())
}
...
async fn main () -> Result<(), Box<dyn Error>> {
    ...
    let matches = App::new("mk_server_cert")
        ...
        .arg(Arg::with_name("region")
         ...
         .arg(Arg::with_name("ip-addr")
             .long("ip-addr")
             .value_name("IP_ADDR")
             .help("IPv4 or IPv6 address to include in the Subject Alt Name extension")
             .takes_value(true)
             .multiple(true)
             .required(false)
             .validator(is_ip_addr))
         .arg(Arg::with_name("dns-name")
             .long("dns-name")
             .value_name("DNS_NAME")
             .help("DNS Name to inclued in the Subject Alt Name extension")
             .takes_value(true)
             .multiple(true)
             .required(false))
        .get_matches();
    ...
    let ip_addrs = matches
        .values_of("ip-addr")
        .map(|ip_addrs| ip_addrs
            .map(|ip_addr| GeneralName::IpAddress(ip_addr.parse::<IpAddr>().unwrap()))
            .collect::<Vec<GeneralName>>())
        .unwrap_or(Vec::with_capacity(0));

    let dns_names = matches
        .values_of("dns-name")
        .map(|dns_names| dns_names
            .map(|dns_name| GeneralName::DnsName(dns_name.to_string()))
            .collect::<Vec<GeneralName>>())
        .unwrap_or(Vec::with_capacity(0));

    let gn = [&ip_addrs[..], &dns_names[..]].concat();
    let san =
        if gn.len() > 0 {
            Some(SubjectAlternativeName(gn))
        } else {
            None
        };
...
//    let pub_key = keypair.public_key();
    ...
    if san.is_some() {
        builder = builder.extension(Extension::from(san.unwrap()));
    }
